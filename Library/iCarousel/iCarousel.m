...
{	
    ...
    {
		[self setup];
        // initWithCoder will not fire in the delegate
        //        [self reloadData];
...
//            NSLog(@"offset: %f, clampedOffset: %f, x: %f, z: %f", offset, clampedOffset, x, z);
...
//    view.superview.center = CGPointMake(self.bounds.size.height/2.0 + contentOffset.height,
//										self.bounds.size.width/2.0 + contentOffset.width);
...
//    NSLog(@"OffsetWidth: %f, OffestHeight: %f", -viewpointOffset.width, -viewpointOffset.height);
...
//        NSLog(@"index: %d - x: %f, y:%f", i, ((CGRect)[view bounds]).origin.x, ((CGRect)[view bounds]).origin.y);
#endif
	}
