...
@synthesize originalView = _originalView, labelWidth = _labelWidth, showNetworkActivityIndicator = _showNetworkActivityIndicator, usingCustomIndicator = _usingCustomIndicator;
...
//    return [[self alloc] initForView:addToView withLabel:labelText width:labelWidth];
    ...
    return [self newActivityViewForView:addToView withLabel:labelText width:0 withImages:nil];
}

/*
 newActivityViewForView:withLabel:width:withImages
 
 Creates and adds an activity view centered within the specified view, using the specified label, a fixed label width, and an array of images for a custom indicator.  The fixed width is useful if you want to change the label text while the view is visible.  Returns the activity view, already added as a subview of the specified view.
 
 Written by TT 2011-07 */

+ (DSActivityView *)newActivityViewForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth withImages:(NSArray *)images;
{
    // Not autoreleased, as it is basically a singleton:
    return [[self alloc] initForView:addToView withLabel:labelText width:labelWidth withImages:images];
}
...
- (DSActivityView *)initForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth withImages:(NSArray *)images;
{
    ...
    // Set the custom flag and create the custom indicator if we are using custom images for the indicator
    if (images != nil) {
        [self setUsingCustomIndicator:YES];
    }
    ...
    if ( [self usingCustomIndicator] ) {
        customActivityIndicator = [self createCustomActivityIndicator:images];
        [self.borderView addSubview:self.customActivityIndicator];
        [self setNeedsLayout];
    }
    else {
...
+ (void)removeView;
{
    NSLog(@"Ending %s: Begin - retainCount: %u", __FUNCTION__, [DSActivityView currentActivityView].retainCount);
    ...
    dsActivityView = nil;
    NSLog(@"Ending %s: End - retainCount: %u", __FUNCTION__, [DSActivityView currentActivityView].retainCount);
}
...
/*
 ...
 Returns a custom activity Indicator that is a UIImageView rather than the UIActivityIndicatorView, creating it if necessary.
...
//    {
//        _customActivityIndicator = [[UIImageView alloc] init];
//        _customActivityIndicator.animationImages = images;                            
...
/*
 initCustomActivityIndicator 
 
*/
- (UIImageView *)createCustomActivityIndicator:(NSArray *)images {
    
    if (!_customActivityIndicator)
    {
        _customActivityIndicator = [[UIImageView alloc] initWithImage: [images objectAtIndex:0]];
    }
    
    if (_customActivityIndicator.isAnimating) {
        [_customActivityIndicator stopAnimating];
    }
        
    _customActivityIndicator.animationImages = [[NSArray alloc] initWithArray:images];
    _customActivityIndicator.animationDuration = 10;
...
/*
 setUsingCustomIndicator:
 
 Sets whether or not we are using a set of custom images to draw the activity indicator
 
 Written by TT 2011-07.
 */

- (void)setUsingCustomIndicator:(BOOL)usingCustomIndicator;
{
    _usingCustomIndicator = usingCustomIndicator;
}
